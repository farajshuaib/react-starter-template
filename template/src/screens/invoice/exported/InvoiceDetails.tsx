...
const InvoiceDetails: React.FC = () => {
  ...
  const getData = async () => {
    ...
    try {
      const { data } = await API.getSpecificExportedInvoice(+params.id);
      setDate(data.data);
      setLoading(false);
    } catch (err) {
      setLoading(false);
          ...
          <div className="bg-white rounded-md my-5">
            <div className="p-8">
              ...
              <div className="grid grid-cols-2 gap-8 mb-5">
                <div className="">
                  <label className="input-label text-gray700">الجهة</label>
                  <div className="my-2 text-xl">{data.authority_name}</div>
                </div>
                <div className="">
                ...
                <div className="">
                  ...
                  <div className="my-2 text-xl">{data.delegate.name}</div>
                </div>
                <div className="">
                  <label className="input-label text-gray700">مندوب فرع</label>
                  <div className="my-2 text-xl">
                    {data.delegate.authority_name}
                  </div>
                </div>
                <div className="">
                  <label className="input-label text-gray700">
